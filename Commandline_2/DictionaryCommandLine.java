...
public class DictionaryCommandLine {
    ...
    public void dictionarySearcher() {
        ...
        int size = dim.dict.wordList.size();

        for(int i = 0; i < size; i++) {
            if (word.equalsIgnoreCase(dim.dict.wordList.get(i).getWord_target().substring(0, length))) {
                System.out.print(dim.dict.wordList.get(i).getWord_target() + ", ");
            }
        }
    }
